...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $pdo->prepare("INSERT INTO interviews_pdf (nom_fichier, description, contenu_fichier, id_metier) VALUES (?, ?, ?, ?)");
    $stmt->execute([$nom_fichier, $description, $contenu_fichier, $id_metier]);
...
<head>
    ...
    <title>Ajout d'un Document PDF pour un Métier</title>
    <link rel="stylesheet" href="../assets/css/bootstrap.min.css">
</head>
<body>
    <div class="container mt-5">
        <h2 class="text-center">Ajout d'un Document PDF pour un Métier</h2>

        <form action="index.php?page=doc_metiers" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name="id_metier" id="id_metier" class="form-control" required>
                    <option value="">Sélectionner un métier</option>
                    <?php
                    try {
                        $query = $pdo->query("SELECT * FROM metiers");
                        $metiers = $query->fetchAll(PDO::FETCH_ASSOC);
                        if (!$metiers) {
                            echo "<option disabled>Aucun métier disponible</option>";
                        } else {
                            foreach ($metiers as $metier) {
                                echo "<option value='" . $metier['id_metier'] . "'>" . htmlspecialchars($metier['nom_metier']) . "</option>";
                            }
                        }
